...
def spd_mx(size):
    a = np.array([[randint(1, 9) for _ in range(size)] for _ in range(size)], dtype='float')
    return np.hstack((a @ a.transpose(), np.array([randint(1, 9) for _ in range(size)])[:, None]))
